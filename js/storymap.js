        ...
        var makeStoryMap = function (element, scenes, layers) {
            ...
            if (settings.navwidget) {
                $.each(sections, function (key, element) {
                    ...
                    if ($(".navbar").length !== 0) {
                        scrollScript = "javascript:window.scrollBy(0, $('section[data-scene=\\'" + section.data('scene') + "\\']').offset().top - $(window).scrollTop() - $('.navbar').height());";
                    } else {
                    ...
                    if (key == 0) {
                        ...
                        $(".navwidget").append('<li><a class="btn btn-default" data-toggle="tooltip" title="' + sceneName + '" href="' + scrollScript + '" >'+sceneName+'</a></li>');
                    }
                });
